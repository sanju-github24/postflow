...
export default function AccountsPage() {
  ...
  const load = async (showLoading = true) => {
    ...
    try {
      ...
      if (Array.isArray(res.data)) {
        ...
        list = res.data.data;
      } else if (res.data?.connected_platforms) {
  ...
  const getStatus = (id: string) => {
    ...
    return statuses.find(
      (s) => String(s.platform).toLowerCase().trim() === id.toLowerCase().trim()
    );
  };
  ...
  const handleDisconnect = async (platformId: string) => {
    ...
    try {
      await fetch(`http://localhost:4000/api/auth/disconnect/${platformId}`, {
        method: "DELETE",
      ...
      <div className="p-8 max-w-2xl mx-auto animate-fade-in">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Manage your social connections and permissions.
          </p>
        </div>
        ...
        {errorMessage && (
          <div className="mb-6 text-sm text-red-700 bg-red-50 border border-red-200 px-4 py-3 rounded-lg animate-in fade-in slide-in-from-top-2">
            {errorMessage}
          </div>
        )}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground mt-0.5 truncate">
                      {isConnected
                        ? `@${status?.username || status?.platform_user_id || "Connected"}`
                        : p.desc}
                    </p>
                    {p.note && !isConnected && (
                      <p className="text-[11px] text-indigo-600 font-medium mt-1 bg-indigo-50 px-2 py-0.5 rounded inline-block">
                        {p.note}
                      </p>
                    )}
                  </div>
