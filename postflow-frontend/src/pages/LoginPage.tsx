...
const PLATFORMS = [
  { label: "Facebook",  logo: "/facebook-logo.png",  color: "#1877F2" },
  { label: "Instagram", logo: "/instagram-logo.png", color: "#E1306C" },
  { label: "Twitter",   logo: "/twitter-logo.png",   color: "#1D9BF0" },
  { label: "LinkedIn",  logo: "/linkedIn-logo.png",  color: "#0A66C2" },
];
          ...
          <div className="bg-white border border-border rounded-2xl shadow-sm p-8">
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center">
                <span className="px-3 bg-white text-xs text-muted-foreground">connect after signing in</span>
              </div>
            </div>

            {/* Platform logos grid — PNG logos */}
            <div className="grid grid-cols-4 gap-2">
              {PLATFORMS.map((p) => (
                <button
                  key={p.label}
                  type="button"
                  onClick={() => setLocalError("Connect your accounts from the Accounts page after signing in")}
                  className="flex flex-col items-center gap-1.5 p-3 rounded-xl border border-border hover:border-indigo-200 hover:bg-indigo-50/50 transition-all"
                >
                  {/* Fixed 28×28 logo container */}
